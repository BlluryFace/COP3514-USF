...
/**********************************************************
 * print: Calculate the cost from the number of nights    *
 *        the dog have stayed.                            *
 **********************************************************/
void cost(Dog list[], int nd){
    string dog_name, owner_name;
    ...
    int i = find_dog(dog_name, owner_name, list, nd);
    if (i >= 0) {
        cout << "The total cost for onboarding is $" << (list[i].get_num_nights() * 10) << endl;
    } else {
        cout << "Dog not found." << std::endl;
    }
}